...
2. Run `python scripts/sync_release_version.py`. It regenerates `uv.lock` and updates the pinned image in `deploy/deploy.yaml` from `pyproject.toml`. The chart's committed `Chart.yaml` uses development metadata; CI injects the release version when packaging, and the image tag defaults to the chart's generated `appVersion`.
